...
use crate::directory_nav_service::dtos::{file_dto::FileDTO, inline_query_dto::InlineQueryDTO};
...
impl FilesDisplayState{
    ...
    pub fn query(&self, query:InlineQueryDTO)->Vec<FileDTO>{
        self.files_display.iter()
        .filter(|dto| dto.name.to_lowercase().contains(&query.query.to_lowercase()))
        .cloned()
        .collect()
    }
}