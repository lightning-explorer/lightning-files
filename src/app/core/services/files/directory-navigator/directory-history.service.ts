...
import { DirectoryMetadata } from "./models/directory-metadata";
...
export class DirectoryHistoryService implements OnDestroy {
    ...
    constructor(private directoryNavService: DirectoryNavigatorService) {
        this.subscription.add(this.directoryNavService.currentDir$.subscribe(dir => {
    ...
    undo() {
        if (this.undoStack.length == 0)
            return // Nowhere to undo
        const lastDirectory = this.undoStack.pop();
        if (this.currentDir)
            this.redoStack.push(this.currentDir);
    ...
    redo() {
        if (this.redoStack.length == 0)
            return // Nowhere to redo
        const nextDirectory = this.redoStack.pop();
        if (this.currentDir)
            this.undoStack.push(this.currentDir);
